...
import { foamProducts } from "@/components/karmo/foam/foamData";
import { group, rise as fade, VIEWPORT } from "@/components/karmo/motion";

const ORANGE = "#FF9A1F";

/**
 * Foam product grid — Home 02 Popular Products tile language.
 * Order Now opens the Karmo product-detail page for that grade.
 */
  ...
  return (
    <motion.div variants={fade} className="group flex flex-col bg-cream/40">
      <div className="relative aspect-square overflow-hidden">
        ...
        <span
          aria-hidden
          className="photo-veil pointer-events-none absolute inset-0 transition-opacity duration-[1300ms] group-hover:opacity-55"
        />
        {onOffer && (
      ...
      <div className="flex flex-1 flex-col items-center px-5 pb-4 pt-3.5 text-center lg:px-6">
        ...
        <p className="mt-1 max-w-[16rem] text-[12px] leading-snug text-ink/45">
          {item.line}
        </p>

        <p className="mb-3.5 mt-2 flex items-baseline justify-center gap-3 leading-none">
          {item.was && (
...
export default function FoamProducts({ categoryId = "all" }) {
  ...
  const items =
    categoryId === "all"
      ? foamProducts
      : foamProducts.filter((p) => p.category === categoryId);

  return (
    <section className="bg-white pb-16 pt-4 lg:pb-24 lg:pt-6">
      <motion.div
        variants={group}
        {...reveal}
        viewport={VIEWPORT}
      ...
      >
        <motion.div variants={fade}>
          ...
          <h2 className="display mt-4 text-[1.9rem] font-light uppercase leading-[1.12] tracking-[0.01em] text-ink lg:text-[2.4rem]">
            Foam products <span className="font-bold text-brand">for you</span>
          </h2>
          <span
            aria-hidden
