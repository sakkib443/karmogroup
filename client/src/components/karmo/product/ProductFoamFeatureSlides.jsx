...
import { foamMorphSlides } from "@/components/karmo/product/foamProductDetailData";
...
export default function ProductFoamFeatureSlides() {
  const reduceMotion = useReducedMotion();
  const sectionRef = useRef(null);
  const firstSwapDone = useRef(false);
  const [index, setIndex] = useState(0);
  const [inView, setInView] = useState(false);
  const slides = foamMorphSlides;
  ...
  return (
    <section
      ...
      aria-label="Comfort on Karmo Foam"
    >
        ...
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-12 xl:gap-16">
            ...
            <div className="mt-4 flex items-center gap-2">
              {slides.map((slide, i) => (
                <button
                  key={slide.id}
                  type="button"
                  aria-label={`Foam comfort pose ${i + 1}`}
          ...
          <div className="max-w-xl lg:justify-self-start">
            ...
            <h2 className="display mt-3 text-[1.65rem] font-bold uppercase leading-[1.12] tracking-[0.02em] text-ink sm:text-[1.95rem] lg:text-[2.15rem]">
              One Karmo foam.
              <span className="mt-1 block text-brand">Three ways to sit.</span>
            </h2>
            <p className="body-copy mt-5 text-[14.5px] leading-[1.7] text-ink/60 sm:text-[15px]">
              Lounge. Read. Rest deep. Same foam density from the same quiet
              room — three ways Bangladesh lives on Karmo.
            </p>
            <ul className="mt-8 space-y-4 border-t border-ink/10 pt-7">
              <li className="body-copy text-[13.5px] leading-[1.55] text-ink/65">
                <span className="font-bold text-ink">Firm, lasting support</span>
                {" — "}
                rubber-grade resilience that holds through everyday use.
              </li>
              <li className="body-copy text-[13.5px] leading-[1.55] text-ink/65">
                <span className="font-bold text-ink">Open airflow</span>
                {" — "}
                cooler seating and cushions made for Bangladesh homes.
              </li>
              <li className="body-copy text-[13.5px] leading-[1.55] text-ink/65">
                <span className="font-bold text-ink">Cut for makers</span>
                {" — "}
                the same craft workshops have trusted since 1965.
              </li>
            </ul>
          </div>
        </div>
