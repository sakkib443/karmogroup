...
import { LuActivity, LuBox, LuCircleCheck, LuCircleX, LuDatabase, LuLoader, LuPlay, LuRefreshCw, LuSearch, LuShield, LuShoppingBag, LuUsers } from 'react-icons/lu';
...
const ApiScannerPage = () => {
    ...
    const [tests, setTests] = useState<ModuleTest[]>([
        { id: 'auth', name: 'Auth & Security', icon: <LuShield />, endpoint: '/users/me', operations: ['READ'], status: 'idle' },
        { id: 'products', name: 'Product Module', icon: <LuBox />, endpoint: '/products', operations: ['READ', 'CREATE'], status: 'idle' },
        { id: 'categories', name: 'Category Module', icon: <LuSearch />, endpoint: '/categories', operations: ['READ'], status: 'idle' },
        { id: 'users', name: 'User Management', icon: <LuUsers />, endpoint: '/users/admin/all', operations: ['READ'], status: 'idle' },
        { id: 'orders', name: 'Order Management', icon: <LuShoppingBag />, endpoint: '/orders/admin/all', operations: ['READ'], status: 'idle' },
        { id: 'analytics', name: 'Analytics System', icon: <LuActivity />, endpoint: '/analytics/dashboard', operations: ['READ'], status: 'idle' },
    ]);
    ...
    const StatusIcon = ({ status }: { status: TestStatus }) => {
        switch (status) {
            case 'running': return <LuLoader className="animate-spin text-blue-500" />;
            case 'success': return <LuCircleCheck className="text-emerald-500" />;
            case 'failed': return <LuCircleX className="text-red-500" />;
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 flex items-center gap-3">
                        <LuDatabase className="text-[var(--color-primary)]" />
                ...
                >
                    {isScanning ? <LuLoader className="animate-spin" /> : <LuPlay />}
                ...
                {tests.map((test) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-6 justify-between md:justify-end">
                            ...
                            >
                                <LuRefreshCw size={18} className={test.status === 'running' ? 'animate-spin' : ''} />
                            </button>
                        </div>
                    </div>
                ))}
